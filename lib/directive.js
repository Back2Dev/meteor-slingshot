 ...
 * @throws Meteor.Error
 *
 * @param {{size: Number, type: String, name: String, id: String}} file - File to be
 * uploaded
...
Slingshot.createDirective = function (name, service, options) {
  ...
  return (Slingshot._directives[name] = new Slingshot.Directive(service, options))
}
...
_.extend(Slingshot.Directive.prototype, {
  ...
  getInstructions: function (method, file, meta) {
    var instructions = this.storageService().upload(method, this._directive, file, meta)
  ...
  requestAuthorization: function (context, file, meta) {
    var validators = Slingshot.Validators,
      restrictions = _.pick(this._directive, ['authorize', 'maxSize', 'allowedFileTypes'])
