  ...
  Object.assign(self, {
    ...
    transfer: function (callback) {
      if (status.curValue !== 'authorized') {
        throw new Error('Cannot transfer file at upload status: ' + status.curValue)
      }
      if (self.file.hasOwnProperty('nativeURL')) {
        //cordova File
      ...
      try {
        ...
        xhr.addEventListener('load', function () {
          if (xhr.status < 400) {
            ...
            status.set('failed')
            callback(getError(), self.file?.id)
          }
        })
        ...
        xhr.addEventListener('abort', function () {
          status.set('aborted')
          callback(new Meteor.Error('Aborted', 'The upload has been aborted by the user'))
        })
